...
describe('IP3Token - Boundary', function () {
    ...
    describe('Test floating point on different fn()', () => {
        ...
        it('Test burn(w/ signature) w/ floating point', async () => {
            ...
            let msg;
            try {
                let input = await IP3Token.connect(owner).populateTransaction[
        ...
        it('Test transferFrom() w/ floating point', async () => {
            ...
            let msg;
            try {
                const inputTransfer = await IP3Token.connect(user1).populateTransaction.transferFrom(
                    owner.address,
                    user2.address,
                    amount
                );
        ...
        it('Test increaseAllowance() w/ floating point', async () => {
            ...
            let msg;
            try {
                const inputIncreaseAllowance = await IP3Token.connect(owner).populateTransaction.increaseAllowance(
                    IP3Token.address,
                    amount
                );
        ...
        it('Test decreaseAllowance() w/ floating point', async () => {
            ...
            let msg;
            try {
                const input = await IP3Token.connect(owner).populateTransaction.decreaseAllowance(
                    IP3Token.address,
                    amount
                );
                msg = await TestHelper.checkResult(input, IP3Token.address, owner, ethers, provider, 0);
    ...
    describe('Test zero(0) number on different fn()', () => {
        ...
        it('Test decreaseAllowance() w/ zero(0) number', async () => {
            const amount = 0;
            const inputAprove = await IP3Token.connect(owner).populateTransaction.approve(
                IP3Token.address,
                ethers.BigNumber.from(10000)
            );
