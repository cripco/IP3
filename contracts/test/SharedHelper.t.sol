...
contract SharedHelper is DSTest {
    ...
    function eip191_burn(
        ...
        if (giveTokens_) IP3Token(_ip3Token).transfer(signer_, amountToBurn_);

        bytes memory signature = eip191_sign_burn(signer_, signerPrivateKey_, amountToBurn_, feeToPay_, nonce_);

        vm.prank(sender_);
        IP3Token(_ip3Token).burn(signer_, amountToBurn_, feeToPay_, nonce_, signature);
    }
    ...
    function eip191_transfer(
        ...
        if (giveTokens_) IP3Token(_ip3Token).transfer(signer_, amountToTransfer_ + feeToPay_);
        ...
        vm.prank(sender_);
        IP3Token(_ip3Token).transfer(signer_, receiver_, amountToTransfer_, feeToPay_, nonce_, signature);
    }
    ...
    function eip191_reserve(
        ...
        if (giveTokens_) IP3Token(_ip3Token).transfer(signer_, amountToReserve_ + feeToPay_);
    ...
    function eip712_permit(
        ...
        vm.prank(sender_);
        IP3Token(_ip3Token).permit(signer_, spender_, amountToPermit_, deadline_, signV, signR, signS);
    }
    ...
    function eip191_burn_verified(
        ...
        else assertEq(IP3Token(_ip3Token).balanceOf(signer_), 0);
